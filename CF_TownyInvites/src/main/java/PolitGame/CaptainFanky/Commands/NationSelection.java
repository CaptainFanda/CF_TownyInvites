...
public class NationSelection extends Cooldown implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if(sender instanceof Player p) {
            ...
            if(town == null) {
                String message = CF_TownyInvites.getMassages().get("not-nation");
                ...
                if(nation != null) {
                    if (isCooldown(p.getUniqueId(), cooldown)) {
                        ...
                        if (resident.isKing()) {
                            ...
                            String message = prefix + CF_TownyInvites.getMassages().get("nationSelection");
                            message = message.replace("%nation%", nation.getName());
                            Bukkit.broadcastMessage(message);
                        } else {
                            String message = prefix + CF_TownyInvites.getMassages().get("not-king");
                            p.sendMessage(message);
                        }
                    }
                } else {
                    String message = prefix + CF_TownyInvites.getMassages().get("not-nation");
                    p.sendMessage(message);
                }
